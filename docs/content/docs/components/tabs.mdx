...
| :--- | :--- | :--- | :--- |
| `variant` | `'primary' \| 'secondary' \| 'tertiary'` | `'primary'` | The visual style of the tabs. |
| `orientation` | `'horizontal' \| 'vertical'` | `'horizontal'` | The layout axis direction of the tab items. |
| `selectedKey` | `Key` | — | Controlled selected key. |
| `defaultSelectedKey` | `Key` | — | Uncontrolled default selected key. |
| `onSelectionChange` | `(key: Key) => void` | — | Callback fired when selecting tabs. |
| `isDisabled` | `boolean` | `false` | Whether interaction is disabled. |

### TabList Props

| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `aria-label` | `string` | — | Accessible name describing the tab list. |
| `items` | `Iterable<T>` | — | Dynamic item collections definition source. |

### Tab Props

| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `id` | `Key` | — | The unique key corresponding to this tab panel. |
| `isDisabled` | `boolean` | `false` | Disables this specific tab. |
| `href` | `string` | — | Makes the tab render as an anchor link to navigate. |
